...
<script>
    ...
    function updateScore(){
        var load = layer.load();
        $.ajax({
            url:'<%=request.getContextPath() %>/admin/update_score.do',
            type:'GET',
            async:false,    //是否异步
            data:{
    ...
    function insertScore(){
        var load = layer.load();
        $.ajax({
            url:'<%=request.getContextPath() %>/admin/insert_score.do',
            type:'GET',
            async:false,    //是否异步
            data:{
                studentId:$("#studentId").val(),
            ...
            if(obj.event === 'detail'){
                ...
                if(false){
                    ...
                    layer.confirm('真的删除行么', {icon: 3}, function(index){
                        var load = layer.load();
                        $.ajax({
                            url:'<%=request.getContextPath() %>/admin/delete_score.do',
                            type:'GET',
                            async:false,    //是否异步
                            data:{
        ...
        var $ = layui.$, active = {
          ...
          <div class="layui-form-item">\
            ...
            <div class="layui-form-item">\
              ...
              <button class="layui-btn" onclick="insertScore()">添加信息</button>\
          </div>\
        '
                });
            },
