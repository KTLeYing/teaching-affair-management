...
<div class="demoTable">
    <span>老师课程列表：</span>
    <button class="layui-btn" data-type="insertTeacherCourse">添加老师课程</button>
    <div class="layui-inline">
        <label>老师id</label>
        <div class="layui-input-inline">
            <input type="text" id="s_teacher_id" class="layui-input">
        </div>
    </div>
    <div class="layui-inline">
        <label>课程id</label>
...
<table class="layui-table" lay-data="{height:471, limit: 10, url:'<%=request.getContextPath() %>/admin/select_teacher_course_list.do', page:true, id:'idTest'}" lay-filter="demo">
    <thead>
    <tr>
        <th lay-data="{field:'id', width:150, sort: true, fixed: true}">ID</th>
        <th lay-data="{field:'teacherId', width:150, sort: true}">老师id</th>
...
<script>
    ...
    function updateTeacherCourse(){
        var load = layer.load();
        $.ajax({
            url:'<%=request.getContextPath() %>/admin/update_teacher_course.do',
            type:'GET',
            async:false,    //是否异步
            data:{
                id:$("#id").val(),
    ...
    var insertIndex;
    function insertTeacherCourse(){
        var load = layer.load();
        $.ajax({
            url:'<%=request.getContextPath() %>/admin/insert_teacher_course.do',
            type:'GET',
            async:false,    //是否异步
            data:{
                id:$("#id").val(),
    ...
    layui.use('table', function(){
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                if(false){
                    ...
                    layer.confirm('真的删除行么', {icon: 3}, function(index){
                        var load = layer.load();
                        $.ajax({
                            url:'<%=request.getContextPath() %>/admin/delete_teacher_course.do',
                            type:'GET',
                            async:false,    //是否异步
                            data:{
                                id:data.id
          ...
          <div class="layui-form layui-form-pane">\
            ...
            <div class="layui-form-item">\
                ...
                <div class="layui-input-block">\
                  <input type="text" id="teacherId" name="teacherId" autocomplete="off" placeholder="请输入老师id" class="layui-input">\
                </div>\
              </div>\
            <div class="layui-form-item">\
            ...
            <div class="layui-form-item">\
              ...
              <button class="layui-btn" onclick="updateTeacherCourse()">修改信息</button>\
          </div>\
        '
                });
                $("#id").val(data.id);
        ...
        var $ = layui.$, active = {
            insertTeacherCourse: function(){ //添加老师课程
          ...
          <div class="layui-form layui-form-pane">\
            ...
            <div class="layui-form-item">\
              ...
              <button class="layui-btn" onclick="insertTeacherCourse()">添加信息</button>\
          </div>\
        '
                });
            },
            refreshTeacherCourse: function(){
                table.reload('idTest', {
                    url: '<%=request.getContextPath() %>/admin/select_teacher_course_list.do'
                    ,where: {} //设定异步数据接口的额外参数
                });
            },
